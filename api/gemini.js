...
function App() {
  const [messages, setMessages] = useState([
    {
      id: 1,
      text: "Olá! Sou o assistente da AC Engenharia. Como posso ajudar com suas cotações hoje?",
      sender: "ai"
    }
  ]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const aiReply = typeof data.reply === "string" ? data.reply : "Sem resposta";
      const aiResponse = { id: Date.now() + 1, text: aiReply, sender: "ai" };
      setMessages(prev => [...prev, aiResponse]);
    } catch (error) {
      console.error("Erro frontend:", error);
      const aiResponse = { id: Date.now() + 1, text: "Erro ao conectar com a IA.", sender: "ai" };
      setMessages(prev => [...prev, aiResponse]);
    } finally {
      setIsLoading(false);
    }
